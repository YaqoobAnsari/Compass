%% 4_experiments
\section{Diagnosing Learned Reconstruction}
...
Holding that budget fixed forces a decision about what a baseline is, and
\cref{tab:main} marks the two kinds apart. RadioUNet, PMNet, RadioDiff and RMDM
follow a specific published architecture, differing only where our input setting
requires it. Daggered entries reproduce no single paper. They are matched-budget
representatives of an architecture family, built to the same conditioning interface
as everything else, and the citation names the work the family is drawn from.
Comparing author implementations would confound the backbone with six training
recipes, whereas the question here is whether the family matters once everything
else is fixed.
...
\subsection{The Non-Line-of-Sight Error Floor}
...
A floor that survives this much architectural variation indicates a missing input
...
\subsection{Interaction Between Occlusion and Cascade Depth}
...
\subsection{Ablating the Transmitter Anchor}
...
The minimum over anchors is what destroys the field, which locates the benefit in
the transmitter's direction rather than in the building layout. The minimum in
\cref{eq:occmeas} is dominated by whichever measurement sits closest to an
unobstructed path, so the field collapses toward zero almost everywhere. Its scene
mean is $0.084$ against $0.298$ for the transmitter-anchored field, and the radial
shadow structure is gone. A transmitter-free geometry channel must therefore
preserve directionality rather than marginalise over it.
