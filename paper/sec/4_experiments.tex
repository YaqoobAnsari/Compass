...
Two kinds of baseline appear in the panel and conflating them would misrepresent
what we ran, so \cref{tab:main} marks the difference. RadioUNet, PMNet, RadioDiff and RMDM are reimplementations of a
...
Geometry conditioning outranks every backbone substitution in \cref{tab:main}, and
two results carry that conclusion. The first is that geometry conditioning takes the cascade from $9.99$ to
...
Redundancy between the two mechanisms is the only reading that fits. A second network in
...
The minimum operator is what destroys the field, as \cref{fig:concept} makes
visible. Taking a minimum over anchors in \cref{eq:occmeas} lets the result be
