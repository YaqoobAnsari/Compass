...
Five boundaries constrain what these results can be taken to show, and we state
them plainly rather than leave them to be found.

\noindent\textbf{A single simulated corpus.} The error floor in \cref{fig:plateau} is
measured on RadioMapSeer, which is ray traced with a single propagation engine. A
floor shared by nine architectures on one corpus is strong evidence about that
...
\noindent\textbf{Two usable measurement sites.} The corpus spans four buildings but
...
\noindent\textbf{Interpolation rather than extrapolation on real data.} Reference points
...
\noindent\textbf{Uncertainty that does not transfer.} Monte Carlo dropout is reasonably
...
\noindent\textbf{Scope of the negative result.}
