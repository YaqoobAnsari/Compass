%% 3_method
\section{Ray-Occlusion Conditioning}
\label{sec:method}
...
zero, with a scene mean of $0.084$ against $0.298$, and removes the directional
structure the channel depends on. The two occlusion panels share one scale.}
\label{fig:concept}
\end{figure*}

\subsection{Problem Formulation}

Radio map reconstruction asks for a dense field over a scene given measurements at a
small subset of its cells, including in the free space that no measurement reached.
We write
...
What separates this from the sparse-to-dense problems vision usually studies is
that the acquisition process, rather than the experimenter, decides which cells are
observed, and it concentrates them along a handful of walks. We therefore draw $k=3$ trajectories of
...
\subsection{The Ray-Occlusion Field}

Received strength depends less on distance than on the material the signal crossed on
the way, and a single scalar per pixel captures that dependence. For a transmitter at $\tau \in \Omega$ and a target cell
...
Although \cref{eq:occ} is fully determined by inputs the network already receives, no
local operator forms it cheaply, which is why we compute it in closed form and supply
it as a channel. A convolution aggregates over a compact neighbourhood, so forming $O$
...
\caption{\textbf{Conditioning and backbones.} Six rasters are concatenated as input
channels. The occlusion field of \cref{eq:occ} is the only one under study and costs
$432$ parameters in the first convolution. The single backbone stops after stage one.
The cascade adds a second network that also receives the coarse map from the first.
Dropout is retained at inference, giving a predictive mean and spread.}
\label{fig:arch}
\end{figure}

\subsection{Backbone Architectures}

Separating the contribution of the channel from the contribution of capacity requires
two backbones of comparable size, shown in \cref{fig:arch}, one of which is the
cascade RadioUNet itself uses. The first is a single encoder-decoder network with $48$ base
...
Beyond that difference in depth the two backbones are conditioned identically, so the
occlusion channel is the only variable that changes across the comparison. Both receive the sparse measurement raster, its binary mask, a coverage
...
\subsection{A Transmitter-Free Variant}
\label{sec:txfree}

The channel's dependence on a known transmitter position is a liability rather than a
detail, because indoor deployments leave that position unrecoverable in most cells.
Indoor cellular deployments place boosters that share cell identifiers with the macro network, so
...
Anchoring the ray at measurement locations instead of at the transmitter removes that
dependence in principle, since a deployment always records where it measured. Let
