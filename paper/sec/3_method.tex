%% 3_method
\section{Geometry as a Conditioning Channel}
\label{sec:method}
...
A radio field assigns a received power to every point of a scene. We write
...
\subsection{The occlusion field}
...
$432$ parameters in the first convolution and no change anywhere else.
...
density map, the building raster and a multi-scale transmitter heatmap. Adding the
occlusion channel is the single variable under study. Dropout is retained at
inference to give a per-pixel predictive spread by Monte Carlo sampling.

\subsection{Removing the transmitter}
\label{sec:txfree}
...
We therefore tried to remove the dependence. Let $\mathcal{A} \subset \mathcal{M}$ be
a sample of measurement locations, which are known without knowing the source.
