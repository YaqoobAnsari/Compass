...
indistinguishable. A floor that survives this much variation is not a capacity
...
passes through building material is a soft shadow depth, fully determined by inputs
the networks already receive. It is nonetheless not something a stack of local
operators computes cheaply, because it is a long thin path-dependent sum between two
points that may be two hundred cells apart. Supplying it as one extra
input channel costs $432$ parameters in a single network and $576$ in the cascade,
and moves both error modes at once, taking line-of-sight error to $3.85$~dB and
non-line-of-sight error to $9.79$~dB, below the band that every other method
shares.

Geometry conditioning and architectural depth turn out to be substitutes rather than
complements, which runs against the usual intuition that better inputs and more
capacity compound. In a controlled $2 \times 2$ design their gains overlap rather than
add. Measured separately they are worth $2.04$ and
$1.51$~dB, and applied together they deliver $2.86$~dB rather than the $3.55$~dB
...
The same finding explains a transfer failure the benchmark literature has not
confronted. The line integral is anchored at the transmitter, and indoor boosters
share cell identifiers with the macro network, so only $17.9\%$ of $67$ measured
cells admit a reliable point source. Removing the anchor by minimising over
measurement locations does not work, because that collapses the field toward zero and
discards the directional structure which made it useful. Classical interpolation
therefore remains ahead of every learned reconstructor we trained on real data, at
$5.51$ against $6.20$~dB with $p = 2.5 \times 10^{-30}$ over $1924$ held-out points.
The gap is not a domain shift that fine-tuning closes. It is an input that one
setting has and the other does not.

The wall attenuation that motivates the channel is visible in the measurements
themselves, at a walled site and not at an open control site. Reference-point pairs
separated by walls differ by an additional $2.31$ to $2.50$~dB at matched distance
with disjoint confidence intervals, while at an open car park used as a negative
control the same measurement returns nothing and cross-validation selects a wall
...
\begin{enumerate}[itemsep=1pt,topsep=3pt,parsep=0pt,leftmargin=1.2em]
    ...
    six architecture families share one non-line-of-sight error floor while differing
    widely in line of sight, which locates the bottleneck in the inputs rather than
    in the models.
    \item A ray-occlusion conditioning channel that lowers that floor for under
    $600$ parameters, and a $2 \times 2$ analysis establishing that it is a substitute for
